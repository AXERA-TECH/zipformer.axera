...
def get_parser():
    ...
    parser.add_argument(
        "--pt-model",
        type=str,
        default="pruned_transducer_stateless7_streaming/exp/epoch-99.pt",
        help="""It specifies the directory where all training related
        files, e.g., checkpoints, log, etc, are saved
        """,
    )
...
def main():
    ...
    if not params.use_averaged_model:
        if params.iter > 0:
            ...
            load_checkpoint(f"{params.pt_model}", model)
        else:
    ...
    try:
        ...
        def simplify_onnx(input_path, output_path):
            model, check = onnxsim.simplify(str(input_path))
            if check:
                onnx.save(model, str(output_path))
                logging.info(f"Simplified ONNX saved to {output_path}")
            else:
                logging.warning(f"ONNX simplification failed for {input_path}")

        encoder_sim = params.exp_dir / f"encoder-{suffix}.sim.onnx"
        decoder_sim = params.exp_dir / f"decoder-{suffix}.sim.onnx"
        joiner_sim = params.exp_dir / f"joiner-{suffix}.sim.onnx"
        simplify_onnx(encoder_filename, encoder_sim)
        simplify_onnx(decoder_filename, decoder_sim)
        simplify_onnx(joiner_filename, joiner_sim)
    except ImportError:
        logging.warning("onnxsim not installed, skipping ONNX simplification.")

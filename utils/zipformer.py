...
class PoolingModule(nn.Module):
    ...
    def streaming_forward(
        ...
        """
        x = x.cumsum(dim=0)  # (T, N, C)
        x = x + (cached_avg * cached_len.unsqueeze(1).to(x.dtype)).unsqueeze(0)
        # Cumulated numbers of frames from start
        cum_mask = torch.arange(1, x.size(0) + 1, device=x.device, dtype=x.dtype)
        cum_mask = cum_mask.unsqueeze(1) + cached_len.unsqueeze(0).to(x.dtype)  # (T, N)
        pooling_mask = (1.0 / cum_mask).unsqueeze(2)
        # now pooling_mask: (T, N, 1)
        x = x * pooling_mask  # (T, N, C)
